...
Таблица уже создана: **«FinApp - Maccaldo»**
(`1dArwQHNH5e5gLaelyWS6og5dossjd5o_HgLNEfLsX18`). В `Code.gs` её ID уже прописан в
`SPREADSHEET_ID`, поэтому скрипт будет писать именно в неё.

1. Откройте https://script.google.com → **Новый проект** (или из таблицы: **Расширения → Apps Script**).
2. Удалите код по умолчанию и вставьте весь файл **`Code.gs`** из этого репозитория.
3. Сверху выберите функцию **`initSheets`** → **Выполнить** (Run). Разрешите доступ,
   когда Google спросит. В таблице появятся все 9 листов и строка владельца.
4. **Развернуть → Новое развёртывание → тип «Веб‑приложение»**:
   - *Выполнять от имени*: **Я**
   - *Доступ*: **Все** (Anyone)
   - **Развернуть** → скопируйте **URL веб‑приложения** (это `GAS_URL`).
5. Проверка: откройте этот URL в браузере — должно вернуться `{"success":true,...}`.

### Шаг 2. Telegram‑бот

Бот уже создан, токен у владельца (вставляется в Railway как `BOT_TOKEN` — **в репозиторий не коммитится**).
Если нужно создать новый: **@BotFather → /newbot** → скопировать токен.
